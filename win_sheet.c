...
void sheet_updown(struct SHTCTL *ctl, struct SHEET *sht, int height) {
    ...
    if (old > height) { //up
        if (height >= 0) {
            for (h = old; h > height; h--) { //调整影响范围的窗口指针顺序
                ctl->sheets[h] = ctl->sheets[h-1];
                ctl->sheets[h]->height = h;
            }
    
            ctl->sheets[height] = sht; //插入当前窗口指针
        ...
        height+1, old);
            ...
            if (ctl->top > old) {
               for (h = old; h < ctl->top; h++) {
                   ctl->sheets[h] = ctl->sheets[h+1]; //这里有bug?? 最后一个越界了??
                   ctl->sheets[h]->height = h;
               }
            }
            ctl->top--;
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1) {
    ...
    for (h = h0; h <= h1; h++) { //依次从最底层往最高层，进行重新绘制窗口（只绘制指定坐标范围内）
        sht = ctl->sheets[h];
        buf = sht->buf;
        sid = sht - ctl->sheets0;
//change here
