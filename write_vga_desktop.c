...
struct AddrRangeDesc {
    unsigned int baseAddrLow; //内存块开始地址的低4字节
    unsigned int baseAddrHigh;//内存块开始地址的高4字节
    unsigned int lengthLow; //内存块长度的低4字节
    unsigned int lengthHigh; //内存块长度的高4字节
    unsigned int type; //内存地址类型 1：表示该内存段可以被内核使用，值范围 0,1,2,3,4,5
};
...
static int mx = 0, my = 0, mmx = -1, mmy = -1; //鼠标位置
...
void CMain(void) {
    ...
    fifo8_init(&keyinfo, 32, keybuf, 0); //初始化键盘缓冲队列
    fifo8_init(&mouseinfo, 128, mousebuf, 0); //初始化鼠标缓冲队列

    init_palette(); //初始化调色板
    init_keyboard(); //初始化键盘-硬件相关
    ...
    int total_memory = 0;
    int displaycnt = 0;
    while (displaycnt < memCnt) {
    	struct AddrRangeDesc* p = memDesc + displaycnt;
    	displaycnt += 1;
    	if (p->type == 1) { //类型为1表示内存可以被内核自由使用
    		total_memory += p->lengthLow;
    	}
    }
    */

    //这里仅分配其中一个内存段进行使用，其他段忽略
    memman_init(memman);
    ...
    //全部可用内存为：0x3FF8F800
    //当前段的内存为：0x3FDE8000
    int memtotal = memman_total(memman)/(1024*1024); //当前机器可以被使用的内存
    ...
    */
    ...
    shtctl = shtctl_init(memman, vram, xsize, ysize); //初始化图层管理器
    sht_back = sheet_alloc(shtctl);  //桌面图层对象
    sht_mouse = sheet_alloc(shtctl); //鼠标图层对象

    buf_back = (unsigned char*)memman_alloc_4k(memman, xsize*ysize); //桌面图层缓冲

    sheet_setbuf(sht_back, buf_back, xsize, ysize, COLOR_INVISIBLE); //关联桌面图层对象和桌面图层缓冲
    sheet_setbuf(sht_mouse, buf_mouse, 16, 16, COLOR_INVISIBLE); //关联鼠标图层对象和鼠标图层缓冲

    init_screen8(buf_back, xsize, ysize); //初始化桌面背景，写入缓冲

    init_mouse_cursor(buf_mouse, COLOR_INVISIBLE);
    sheet_slide(shtctl, sht_back, 0, 0); //设置桌面左上角显示 (0,0)

    mx = (xsize - 16) / 2;
    my = (ysize - 28 - 16) / 2;
    sheet_slide(shtctl, sht_mouse, mx, my); //设置鼠标左上角显示 (mx,my)
